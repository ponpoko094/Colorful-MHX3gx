...
std::vector<std::string> listPalicoSkill{"なし",
                                         "体力強化の術【小】",
                                         "体力強化の術【大】",
                                         "攻撃強化の術【小】",
                                         "攻撃強化の術【大】",
...
std::vector<std::string> listPalicoAction{"なし",
                                          "薬草笛の技",
                                          "回復笛の技",
                                          "真・回復笛の技",
                                          "解毒・消臭笛の技",
...
// スーパーノヴァ連射等
void NoMotion(MenuEntry *entry) { KBD::Toggle32("モーションを無くしますか？", 0xAF55A8, 0xE3A00001, 0x13A00001); }

// クエスト時間停止
void QuestTimeStop(MenuEntry *entry) { KBD::Toggle32("クエスト時間を停止しますか？", 0x90E5BC, 0xEA000008, 0xBA000008); }
...
void HunterNameChange(MenuEntry *entry) {
    ...
    Keyboard keyboard("どちらで変更しますか？\n現在の名前[" + hunterNameNow + "]", listFixKeyboard);
...
void HaveItem99(MenuEntry *entry) {
    ...
    if (choice == 1) {
        for (int i = 0; i < 24; i++) {
            Process::Write32(i * 4 + 0xC18DB0, 0);
        }
...
void ChargeSpeedUp(MenuEntry *entry) {
    ...
    if (choice == 1) {
        for (int i = 0; i < 11; i++) {
            Process::Write32(i * 4 + 0xC18E24, 0);
        }
...
void PlayerSpeedOption(MenuEntry *entry) {
    std::vector<std::string> list1to10Magnification{"1倍", "2倍", "3倍", "4倍", "5倍", "6倍", "7倍", "8倍", "9倍", "10倍"};
...
void PlayerAttackPowerMagnificationChange(MenuEntry *entry) {
    if (Controller::IsKeysPressed(Y + KEY_DU)) {
        ...
        Process::Write8(0xC18E58, playerAttackPowerMagnification);
...
void SkinColorChange(MenuEntry *entry) {
    std::vector<std::string> listSkinColor{"赤色", "橙色", "黄色", "黄緑", "緑色", "緑水", "水色", "水青", "青色", "紫色", "紫桃", "白色", "灰色", "黒色"};
...
void AmuletTypeChange(MenuEntry *entry) {
    std::vector<std::string> listAmuletType{"表示無し", "兵士の護石", "闘士の護石", "騎士の護石", "城塞の護石", "女王の護石", "王の護石", "龍の護石"};
...
void AmuletSkillChange(MenuEntry *entry) {
    std::vector<std::string> listAmuletSkill{
        "スキル無し", "毒",         "麻痺",       "睡眠",     "気絶",       "聴覚保護",   "風圧",       "耐震",       "だるま",     "耐暑",
        "耐寒",       "寒冷適応",   "炎熱適応",   "盗み無効", "対防御DOWN", "狂撃耐性",   "細菌学",     "裂傷",       "攻撃",       "防御",
        "体力",       "火耐性",     "水耐性",     "雷耐性",   "氷耐性",     "龍耐性",     "属性耐性",   "火属性攻撃", "水属性攻撃", "雷属性攻撃",
        "氷属性攻撃", "龍属性攻撃", "属性攻撃",   "特殊攻撃", "研ぎ師",     "匠",         "斬れ味",     "剣術",       "研磨術",     "鈍器",
        "抜刀会心",   "抜刀滅気",   "納刀",       "装填速度", "反動",       "精密射撃",   "通常弾強化", "貫通弾強化", "散弾強化",   "重撃弾強化",
        "通常弾追加", "貫通弾追加", "散弾追加",   "榴弾追加", "拡散弾追加", "毒瓶追加",   "麻痺瓶追加", "睡眠瓶追加", "強撃瓶追加", "属強瓶追加",
        "接撃瓶追加", "滅気瓶追加", "爆破瓶追加", "速射",     "射法",       "装填数",     "変則射撃",   "弾薬節約",   "達人",       "痛撃",
        "連撃",       "特殊会心",   "属性会心",   "会心強化", "溜め短縮",   "スタミナ",   "体術",       "気力回復",   "回避性能",   "回避距離",
        "泡沫",       "ガード性能", "ガード強化", "KO",       "滅気攻撃",   "笛",         "砲術",       "重撃",       "爆弾強化",   "本気",
        "闘魂",       "無傷",       "チャンス",   "底力",     "逆境",       "逆上",       "窮地",       "根性",       "気配",       "采配",
        "号令",       "乗り",       "跳躍",       "無心",     "千里眼",     "観察眼",     "狩人",       "運搬",       "加護",       "英雄の盾",
        "回復量",     "回復速度",   "効果持続",   "広域",     "腹減り",     "食いしん坊", "食事",       "節食",       "肉食",       "茸食",
        "野草知識",   "調合成功率", "調合数",     "高速設置", "採取",       "ハチミツ",   "護石王",     "気まぐれ",   "運気",       "剥ぎ取り",
        "捕獲",       "ベルナ",     "ココット",   "ポッケ",   "ユクモ",     "紅兜",       "大雪主",     "矛砕",       "岩穿",       "紫毒姫",
        "宝纏",       "白疾風",     "隻眼",       "黒炎王",   "金雷公",     "荒鉤爪",     "燼滅刃",     "北辰納豆流", "胴系統倍加"};
...
void AmuletSlotChange(MenuEntry *entry) {
    std::vector<std::string> listAmuletSlot{"0スロット", "1スロット", "2スロット", "3スロット"};
...
void OtherPlayerEquipmentCopy(MenuEntry *entry) {
    ...
    Keyboard keyboard("装備をコピーしたいプレイヤーを選んで下さい", list1to4Player);
    int choice = keyboard.Open();
...
void SpecialPermitQuestTicketChange(MenuEntry *entry) {
    std::vector<std::string> listSpecialTicket{"紅兜アオアシラ",   "大雪主ウルクスス",   "矛砕ダイミョウザザミ", "紫毒姫リオレイア",
                                               "岩穿テツカブラ",   "白疾風ナルガクルガ", "宝纏ウラガンキン",     "隻眼イャンガルルガ",
                                               "黒炎王リオレウス", "金雷公ジンオウガ",   "荒鉤爪ティガレックス", "燼滅刃ディノバルド"};
...
void VillageContributionPointChange(MenuEntry *entry) {
    ...
    std::vector<std::string> listVillage{"ベルナ村", "ココット村", "ポッケ村", "ユクモ村"};
...
void RoomServiceChange(MenuEntry *entry) {
    std::vector<std::string> listRoomService{"ルームサービス", "キャラバンの看板娘", "モガの村の看板娘", "タンジアの港の看板娘", "ぽかぽか島の管理人"};
...
void GuildCardChange(MenuEntry *entry) {
    std::vector<std::string> listGuildCardChangeGroup{"称号",         "クリア回数", "すれ違い回数",      "友好度", "背景", "ポーズ",
                                                      "武器使用回数", "プレイ時間", "モンスター狩猟記録"};

    std::vector<std::string> listGuildCardChangeTitle{"称号1", "称号2", "称号3"};

    std::vector<std::string> listGuildCardChangeClearValue{"村", "集会所下位", "集会所上位", "特殊許可クエスト", "闘技大会"};

    std::vector<std::string> listGuildCardChangeWeaponUseGroup{"村", "集会所", "闘技大会", "全てカンスト"};

    std::vector<std::string> listGuildCardChangeWeaponUseType{
        "大剣",       "片手剣", "ハンマー", "ランス", "ヘビィボウガン", "ライトボウガン",   "太刀",      "スラッシュアックス",
        "ガンランス", "弓",     "双剣",     "狩猟笛", "操虫棍",         "チャージアックス", "ニャンター"};

    std::vector<std::string> listGuildCardChangeMonsterHuntingGroup{"狩猟数", "捕獲数", "どちらもカンスト"};

    std::vector<std::string> listGuildCardChangeMonsterHuntingBigSmall{"大型モンスター", "小型モンスター"};

    std::vector<std::string> listGuildCardChangeMonsterHuntingBig{"リオレイア",
                                                                  "リオレイア希少種",
                                                                  "紫毒姫リオレイア",
                                                                  "リオレウス",
                                                                  "リオレウス希少種",
    ...
    std::vector<std::string> listGuildCardChangeMonsterHuntingSmall{
        "アプトノス", "アプケロス", "ケルビ",     "モス",         "カンタロス", "ランゴスタ", "アイルー",     "メラルー", "ランポス",
        "ゲネポス",   "イーオス",   "ガレオス",   "ブルファンゴ", "ポポ",       "ギアノス",   "ガウシカ",     "ガブラス", "ヤオザミ",
        "ガミザミ",   "ブランゴ",   "リノプロス", "ブナハブラ",   "オルタロス", "ジャギィ",   "ジャギィノス", "ルドロス", "ウロコトル",
        "ズワロポス", "ガーグァ",   "スクアギル", "クンチュウ",   "マッカォ",   "リモセトス", "ムーファ"};
    ...
    int result = keyboard.Open();
    if (result == 0) {
        Keyboard keyboard("下画面の説明文に変更する称号を選んでください。", listGuildCardChangeTitle);
    ...
    if (result == 1) {
        Keyboard keyboard("変更するクリア回数を選んでください。", listGuildCardChangeClearValue);
    ...
    if (result == 6) {
        Keyboard keyboard("グループを選んでください。", listGuildCardChangeWeaponUseGroup);
        int gurupu = keyboard.Open();
        if (gurupu >= 0 && gurupu <= 2) {
            Keyboard keyboard("武器種を選んでください。", listGuildCardChangeWeaponUseType);
            int choice = keyboard.Open();
            if (choice >= 0) {
    ...
    if (result == 8) {
        Keyboard keyboard("グループを選んでください。", listGuildCardChangeMonsterHuntingGroup);
        int huntingGroup = keyboard.Open();
        if (huntingGroup == 0) {
            Keyboard keyboard("ページを選んでください。\n(ギルドカードの並びです)", listGuildCardChangeMonsterHuntingBigSmall);
            int huntPage = keyboard.Open();
            if (huntPage == 0) {
                Keyboard monsterChoice("モンスターを選んでください。", listGuildCardChangeMonsterHuntingBig);
            ...
            if (huntPage == 1) {
                Keyboard monsterChoice("小型モンスターを選んでください。", listGuildCardChangeMonsterHuntingSmall);
...
void ItemBoxEdit(MenuEntry *entry) {
    u16 data16;
    std::vector<std::string> listItemBoxEdit{"アイテムを入れる[1400種]", "アイテムを入れる[548種]", "全アイテム99個", "全アイテム消去"};
...
void ChageStageOption(MenuEntry *entry) {
    std::vector<std::string> listStageSelect{"0段階目", "1段階目", "2段階目", "3段階目", "4段階目"};
...
// クエスト最大ダウン回数
void QuestDownMaxChange(MenuEntry *entry) { Process::Write8(0x8365440, quedownmax); }
...
// クエスト現在ダウン回数
void QuestDownNowChange(MenuEntry *entry) { Process::Write8(0x8365440, quedownnow); }
...
void QuestTimeDisplay(MenuEntry *entry) {
    ...
    if (questFrame1 != 0) {
        ...
        OSD::Run([](const Screen &screen) {
            ...
            if (addressOn == true) {
                if (questFrame1 <= 3600) {
                    screen.Draw(Utils::Format("QT %d:%02d:%02d:%02d", questHour2, questMinute2, questSecond2, questFrame2), 10, 180, Color::Red);
                    addressOn = false;
                } else {
                    screen.Draw(Utils::Format("QT %d:%02d:%02d:%02d", questHour2, questMinute2, questSecond2, questFrame2), 10, 180, Color::Yellow);
                    addressOn = false;
                }
            }
            return (true);
...
void AllQuestClearChange(MenuEntry *entry) {
    std::vector<std::string> listAllQuestClearSelect{"クエスト全クリア", "クエスト全未クリア"};
...
void FenyAndPugyClothes(MenuEntry *entry) {
    std::vector<std::string> listFenyClothes{"ルンルンベル", "愛しのマドモワゼル", "召しませ姫林檎", "常夏リゾート"};

    std::vector<std::string> listPugyClothes{
        "思い出ストライプ", "夢追いアミーゴ",   "はだかの王様",     "魅惑のピンク",       "緑と黒の衝撃",     "眠りを誘う白",   "ドリームスター",
        "天使のレオタード", "カエルのマーチ",   "ひよっこアフロ",   "ブラックハニー",     "純真クレナイ",     "おシノビ裝束",   "マフモフウォーマー",
        "ブンブンビー",     "ファンキーアフロ", "いやされムーチョ", "さすらい道中",       "チューボー見習い", "あやかし装束",   "あまくちピンク",
        "なき虫ベイビー",   "なついろ恋浴衣",   "モスのきもち",     "ルドロスウォーマー", "ウーパールンバ",   "イビルのきもち", "ドスのきもち",
        "ハートの女王様",   "パンプキンナイト", "キラキラ★ナイト"};
...
void PlayerVoiceChange(MenuEntry *entry) {
    u8 currentVoice;
    std::vector<std::string> listVoiceType{"声なし",  "TYPE 1",  "TYPE 2",  "TYPE 3",  "TYPE 4",  "TYPE 5",  "TYPE 6",
...
void PlayerEyeColorChange(MenuEntry *entry) {
    u8 currentEye;
    std::vector<std::string> listEyeType{"茶色", "赤色", "青色", "黄色", "緑色", "紫色", "白色", "黒色", "全部白色", "全部黒色"};
...
void PlayerInnerChange(MenuEntry *entry) {
    u8 currentInner;
    std::vector<std::string> listInnerType{"TYPE 1", "TYPE 2", "TYPE 3", "TYPE 4", "TYPE 5", "TYPE 6", "TYPE 7"};
...
void PlayerHuntingStyleChange(MenuEntry *entry) {
    ...
    std::vector<std::string> listHuntingStyle{"ギルドスタイル", "ストライカースタイル", "エリアルスタイル", "ブシドースタイル"};
...
void PlayerHearStyleChange(MenuEntry *entry) {
    u8 currentHearStyle;
    std::vector<std::string> listHearStyleType{"TYPE 1",  "TYPE 2",  "TYPE 3",  "TYPE 4",  "TYPE 5",  "TYPE 6",  "TYPE 7",  "TYPE 8",  "TYPE 9",  "TYPE 10",
                                               "TYPE 11", "TYPE 12", "TYPE 13", "TYPE 14", "TYPE 15", "TYPE 16", "TYPE 17", "TYPE 18", "TYPE 19", "TYPE 20",
                                               "TYPE 21", "TYPE 22", "TYPE 23", "TYPE 24", "TYPE 25", "TYPE 26", "TYPE 27", "TYPE 28", "TYPE 29", "TYPE 30"};
...
void PlayerFaceChange(MenuEntry *entry) {
    u8 currentFace;
    std::vector<std::string> listFaceType{"TYPE 1",  "TYPE 2",  "TYPE 3",  "TYPE 4",  "TYPE 5",  "TYPE 6",  "TYPE 7",  "TYPE 8",  "TYPE 9",
                                          "TYPE 10", "TYPE 11", "TYPE 12", "TYPE 13", "TYPE 14", "TYPE 15", "TYPE 16", "TYPE 17", "TYPE 18"};
...
void PlayerCosmeticsChange(MenuEntry *entry) {
    u8 currentCosmetics;
    std::vector<std::string> listMakeType{"TYPE 1", "TYPE 2",  "TYPE 3",  "TYPE 4",  "TYPE 5",  "TYPE 6",  "TYPE 7",    "TYPE 8",
                                          "TYPE 9", "TYPE 10", "TYPE 11", "TYPE 12", "TYPE 13", "TYPE 14", "メイクなし"};
...
void Information(MenuEntry *entry) {
    StringVector info{"タイトルID", "プロセスの名前", "3DSのタイプ", "Wi-Fi接続確認", "バッテリー残量確認"};
...
void ChatConversionChange(MenuEntry *entry) {
    ...
    if (Controller::IsKeysDown(R)) {
        if (character1 == "るーと") Process::WriteString(aaa, "√", StringFormat::Utf16);
        if (character2 == "るーと") Process::WriteString(bbb, "√", StringFormat::Utf16);
        if (character3 == "るーと") Process::WriteString(ccc, "√", StringFormat::Utf16);
        if (character1 == "まるいち") Process::WriteString(aaa, "①", StringFormat::Utf16);
        if (character2 == "まるいち") Process::WriteString(bbb, "①", StringFormat::Utf16);
        if (character3 == "まるいち") Process::WriteString(ccc, "①", StringFormat::Utf16);
        if (character1 == "まるに") Process::WriteString(aaa, "②", StringFormat::Utf16);
        if (character2 == "まるに") Process::WriteString(bbb, "②", StringFormat::Utf16);
        if (character3 == "まるに") Process::WriteString(ccc, "②", StringFormat::Utf16);
        if (character1 == "まるさん") Process::WriteString(aaa, "③", StringFormat::Utf16);
        if (character2 == "まるさん") Process::WriteString(bbb, "③", StringFormat::Utf16);
        if (character3 == "まるさん") Process::WriteString(ccc, "③", StringFormat::Utf16);
        if (character1 == "まるよん") Process::WriteString(aaa, "④", StringFormat::Utf16);
        if (character2 == "まるよん") Process::WriteString(bbb, "④", StringFormat::Utf16);
        if (character3 == "まるよん") Process::WriteString(ccc, "④", StringFormat::Utf16);
        if (character1 == "まるご") Process::WriteString(aaa, "⑤", StringFormat::Utf16);
        if (character2 == "まるご") Process::WriteString(bbb, "⑤", StringFormat::Utf16);
        if (character3 == "まるご") Process::WriteString(ccc, "⑤", StringFormat::Utf16);
        if (character1 == "まるろく") Process::WriteString(aaa, "⑥", StringFormat::Utf16);
        if (character2 == "まるろく") Process::WriteString(bbb, "⑥", StringFormat::Utf16);
        if (character3 == "まるろく") Process::WriteString(ccc, "⑥", StringFormat::Utf16);
        if (character1 == "まるなな") Process::WriteString(aaa, "⑦", StringFormat::Utf16);
        if (character2 == "まるなな") Process::WriteString(bbb, "⑦", StringFormat::Utf16);
        if (character3 == "まるなな") Process::WriteString(ccc, "⑦", StringFormat::Utf16);
        if (character1 == "まるはち") Process::WriteString(aaa, "⑧", StringFormat::Utf16);
        if (character2 == "まるはち") Process::WriteString(bbb, "⑧", StringFormat::Utf16);
        if (character3 == "まるはち") Process::WriteString(ccc, "⑧", StringFormat::Utf16);
        if (character1 == "まるきゅう") Process::WriteString(aaa, "⑨", StringFormat::Utf16);
        if (character2 == "まるきゅう") Process::WriteString(bbb, "⑨", StringFormat::Utf16);
        if (character3 == "まるきゅう") Process::WriteString(ccc, "⑨", StringFormat::Utf16);
        if (character1 == "えす") Process::WriteString(aaa, "∫", StringFormat::Utf16);
        if (character2 == "えす") Process::WriteString(bbb, "∫", StringFormat::Utf16);
        if (character3 == "えす") Process::WriteString(ccc, "∫", StringFormat::Utf16);
        if (character1 == "だぶるえす") Process::WriteString(aaa, "∬", StringFormat::Utf16);
        if (character2 == "だぶるえす") Process::WriteString(bbb, "∬", StringFormat::Utf16);
        if (character3 == "だぶるえす") Process::WriteString(ccc, "∬", StringFormat::Utf16);
        if (character1 == "ろーまいち") Process::WriteString(aaa, "Ⅰ", StringFormat::Utf16);
        if (character2 == "ろーまいち") Process::WriteString(bbb, "Ⅰ", StringFormat::Utf16);
        if (character3 == "ろーまいち") Process::WriteString(ccc, "Ⅰ", StringFormat::Utf16);
        if (character1 == "ろーまに") Process::WriteString(aaa, "Ⅱ", StringFormat::Utf16);
        if (character2 == "ろーまに") Process::WriteString(bbb, "Ⅱ", StringFormat::Utf16);
        if (character3 == "ろーまに") Process::WriteString(ccc, "Ⅱ", StringFormat::Utf16);
        if (character1 == "ろーまさん") Process::WriteString(aaa, "Ⅲ", StringFormat::Utf16);
        if (character2 == "ろーまさん") Process::WriteString(bbb, "Ⅲ", StringFormat::Utf16);
        if (character3 == "ろーまさん") Process::WriteString(ccc, "Ⅲ", StringFormat::Utf16);
        if (character1 == "ろーまよん") Process::WriteString(aaa, "Ⅳ", StringFormat::Utf16);
        if (character2 == "ろーまよん") Process::WriteString(bbb, "Ⅳ", StringFormat::Utf16);
        if (character3 == "ろーまよん") Process::WriteString(ccc, "Ⅳ", StringFormat::Utf16);
        if (character1 == "ろーまご") Process::WriteString(aaa, "Ⅴ", StringFormat::Utf16);
        if (character2 == "ろーまご") Process::WriteString(bbb, "Ⅴ", StringFormat::Utf16);
        if (character3 == "ろーまご") Process::WriteString(ccc, "Ⅴ", StringFormat::Utf16);
        if (character1 == "ろーまろく") Process::WriteString(aaa, "Ⅵ", StringFormat::Utf16);
        if (character2 == "ろーまろく") Process::WriteString(bbb, "Ⅵ", StringFormat::Utf16);
        if (character3 == "ろーまろく") Process::WriteString(ccc, "Ⅵ", StringFormat::Utf16);
        if (character1 == "ろーまなな") Process::WriteString(aaa, "Ⅶ", StringFormat::Utf16);
        if (character2 == "ろーまなな") Process::WriteString(bbb, "Ⅶ", StringFormat::Utf16);
        if (character3 == "ろーまなな") Process::WriteString(ccc, "Ⅶ", StringFormat::Utf16);
        if (character1 == "ろーまはち") Process::WriteString(aaa, "Ⅷ", StringFormat::Utf16);
        if (character2 == "ろーまはち") Process::WriteString(bbb, "Ⅷ", StringFormat::Utf16);
        if (character3 == "ろーまはち") Process::WriteString(ccc, "Ⅷ", StringFormat::Utf16);
        if (character1 == "ろーまきゅう") Process::WriteString(aaa, "Ⅸ", StringFormat::Utf16);
        if (character2 == "ろーまきゅう") Process::WriteString(bbb, "Ⅸ", StringFormat::Utf16);
        if (character3 == "ろーまきゅう") Process::WriteString(ccc, "Ⅸ", StringFormat::Utf16);
        if (character1 == "ろーまじゅう") Process::WriteString(aaa, "Ⅹ", StringFormat::Utf16);
        if (character2 == "ろーまじゅう") Process::WriteString(bbb, "Ⅹ", StringFormat::Utf16);
        if (character3 == "ろーまじゅう") Process::WriteString(ccc, "Ⅹ", StringFormat::Utf16);
        if (character1 == "みり") Process::WriteString(aaa, "㍉", StringFormat::Utf16);
        if (character2 == "みり") Process::WriteString(bbb, "㍉", StringFormat::Utf16);
        if (character3 == "みり") Process::WriteString(ccc, "㍉", StringFormat::Utf16);
        if (character1 == "きろ") Process::WriteString(aaa, "㌔", StringFormat::Utf16);
        if (character2 == "きろ") Process::WriteString(bbb, "㌔", StringFormat::Utf16);
        if (character3 == "きろ") Process::WriteString(ccc, "㌔", StringFormat::Utf16);
        if (character1 == "せんち") Process::WriteString(aaa, "㌢", StringFormat::Utf16);
        if (character2 == "せんち") Process::WriteString(bbb, "㌢", StringFormat::Utf16);
        if (character3 == "せんち") Process::WriteString(ccc, "㌢", StringFormat::Utf16);
        if (character1 == "めーとる") Process::WriteString(aaa, "㍍", StringFormat::Utf16);
        if (character2 == "めーとる") Process::WriteString(bbb, "㍍", StringFormat::Utf16);
        if (character3 == "めーとる") Process::WriteString(ccc, "㍍", StringFormat::Utf16);
        if (character1 == "ぐらむ") Process::WriteString(aaa, "㌘", StringFormat::Utf16);
        if (character2 == "ぐらむ") Process::WriteString(bbb, "㌘", StringFormat::Utf16);
        if (character3 == "ぐらむ") Process::WriteString(ccc, "㌘", StringFormat::Utf16);
        if (character1 == "とん") Process::WriteString(aaa, "㌧", StringFormat::Utf16);
        if (character2 == "とん") Process::WriteString(bbb, "㌧", StringFormat::Utf16);
        if (character3 == "とん") Process::WriteString(ccc, "㌧", StringFormat::Utf16);
        if (character1 == "あーる") Process::WriteString(aaa, "㌃", StringFormat::Utf16);
        if (character2 == "あーる") Process::WriteString(bbb, "㌃", StringFormat::Utf16);
        if (character3 == "あーる") Process::WriteString(ccc, "㌃", StringFormat::Utf16);
        if (character1 == "へくたーる") Process::WriteString(aaa, "㌶", StringFormat::Utf16);
        if (character2 == "へくたーる") Process::WriteString(bbb, "㌶", StringFormat::Utf16);
        if (character3 == "へくたーる") Process::WriteString(ccc, "㌶", StringFormat::Utf16);
        if (character1 == "りっとる") Process::WriteString(aaa, "㍑", StringFormat::Utf16);
        if (character2 == "りっとる") Process::WriteString(bbb, "㍑", StringFormat::Utf16);
        if (character3 == "りっとる") Process::WriteString(ccc, "㍑", StringFormat::Utf16);
        if (character1 == "わっと") Process::WriteString(aaa, "㍗", StringFormat::Utf16);
        if (character2 == "わっと") Process::WriteString(bbb, "㍗", StringFormat::Utf16);
        if (character3 == "わっと") Process::WriteString(ccc, "㍗", StringFormat::Utf16);
        if (character1 == "かろりー") Process::WriteString(aaa, "㌍", StringFormat::Utf16);
        if (character2 == "かろりー") Process::WriteString(bbb, "㌍", StringFormat::Utf16);
        if (character3 == "かろりー") Process::WriteString(ccc, "㌍", StringFormat::Utf16);
        if (character1 == "どる") Process::WriteString(aaa, "㌦", StringFormat::Utf16);
        if (character2 == "どる") Process::WriteString(bbb, "㌦", StringFormat::Utf16);
        if (character3 == "どる") Process::WriteString(ccc, "㌦", StringFormat::Utf16);
        if (character1 == "せんと") Process::WriteString(aaa, "㌣", StringFormat::Utf16);
        if (character2 == "せんと") Process::WriteString(bbb, "㌣", StringFormat::Utf16);
        if (character3 == "せんと") Process::WriteString(ccc, "㌣", StringFormat::Utf16);
        if (character1 == "ぱーせんと") Process::WriteString(aaa, "㌫", StringFormat::Utf16);
        if (character2 == "ぱーせんと") Process::WriteString(bbb, "㌫", StringFormat::Utf16);
        if (character3 == "ぱーせんと") Process::WriteString(ccc, "㌫", StringFormat::Utf16);
        if (character1 == "みりばーる") Process::WriteString(aaa, "㍊", StringFormat::Utf16);
        if (character2 == "みりばーる") Process::WriteString(bbb, "㍊", StringFormat::Utf16);
        if (character3 == "みりばーる") Process::WriteString(ccc, "㍊", StringFormat::Utf16);
        if (character1 == "ぺーじ") Process::WriteString(aaa, "㌻", StringFormat::Utf16);
        if (character2 == "ぺーじ") Process::WriteString(bbb, "㌻", StringFormat::Utf16);
        if (character3 == "ぺーじ") Process::WriteString(ccc, "㌻", StringFormat::Utf16);
        if (character1 == "へいせい") Process::WriteString(aaa, "㍻", StringFormat::Utf16);
        if (character2 == "へいせい") Process::WriteString(bbb, "㍻", StringFormat::Utf16);
        if (character3 == "へいせい") Process::WriteString(ccc, "㍻", StringFormat::Utf16);
        if (character1 == "しょうわ") Process::WriteString(aaa, "㍼", StringFormat::Utf16);
        if (character2 == "しょうわ") Process::WriteString(bbb, "㍼", StringFormat::Utf16);
        if (character3 == "しょうわ") Process::WriteString(ccc, "㍼", StringFormat::Utf16);
        if (character1 == "たいしょう") Process::WriteString(aaa, "㍽", StringFormat::Utf16);
        if (character2 == "たいしょう") Process::WriteString(bbb, "㍽", StringFormat::Utf16);
        if (character3 == "たいしょう") Process::WriteString(ccc, "㍽", StringFormat::Utf16);
        if (character1 == "めいじ") Process::WriteString(aaa, "㍾", StringFormat::Utf16);
        if (character2 == "めいじ") Process::WriteString(bbb, "㍾", StringFormat::Utf16);
        if (character3 == "めいじ") Process::WriteString(ccc, "㍾", StringFormat::Utf16);
        if (character1 == "なんばー") Process::WriteString(aaa, "№", StringFormat::Utf16);
        if (character2 == "なんばー") Process::WriteString(bbb, "№", StringFormat::Utf16);
        if (character3 == "なんばー") Process::WriteString(ccc, "№", StringFormat::Utf16);
        if (character1 == "けーけー") Process::WriteString(aaa, "㏍", StringFormat::Utf16);
        if (character2 == "けーけー") Process::WriteString(bbb, "㏍", StringFormat::Utf16);
        if (character3 == "けーけー") Process::WriteString(ccc, "㏍", StringFormat::Utf16);
        if (character1 == "てる") Process::WriteString(aaa, "℡", StringFormat::Utf16);
        if (character2 == "てる") Process::WriteString(bbb, "℡", StringFormat::Utf16);
        if (character3 == "てる") Process::WriteString(ccc, "℡", StringFormat::Utf16);
        if (character1 == "たぶ") Process::WriteString(aaa, "\t", StringFormat::Utf16);
        if (character2 == "たぶ") Process::WriteString(bbb, "\t", StringFormat::Utf16);
        if (character3 == "たぶ") Process::WriteString(ccc, "\t", StringFormat::Utf16);
        if (character1 == "かいぎょう") Process::WriteString(aaa, "\n", StringFormat::Utf16);
        if (character2 == "かいぎょう") Process::WriteString(bbb, "\n", StringFormat::Utf16);
        if (character3 == "かいぎょう") Process::WriteString(ccc, "\n", StringFormat::Utf16);
        if (character1 == "こう") Process::WriteString(aaa, "爻", StringFormat::Utf16);
        if (character2 == "こう") Process::WriteString(bbb, "爻", StringFormat::Utf16);
        if (character3 == "こう") Process::WriteString(ccc, "爻", StringFormat::Utf16);
...
void ChatConversionList(MenuEntry *entry) {
    std::vector<std::string> listChatConversion{
        "るーと √",       "えす ∫",           "だぶるえす ∬", "なんばー №",   "けーけー ㏍", "てる ℡", "こう 爻",
        "まる(数字) ① ②", "ろーま(数字) Ⅰ Ⅱ", "(単位) ㌔ ㍍", "(元号) ㍻ ㍼", "かいぎょう",  "たぶ",   "どんどん追加してくよ！"};
...
void PalicoSupportTrendChange(MenuEntry *entry) {
    u8 sup;
    std::vector<std::string> listPalicoSupportTrend{"カリスマ", "ファイト", "ガード", "アシスト", "回復", "ボマー", "��レクト"};
...
void PalicoTargetChange(MenuEntry *entry) {
    u8 tar;
    std::vector<std::string> listPalicoTarget{"指定なし", "小型一筋", "小型優先", "バランス", "大型優先", "大型一筋"};
...
void PalicoInnerChange(MenuEntry *entry) {
    u8 inner;
    std::vector<std::string> listPalicoInner{"TYPE1", "TYPE2", "ファラオ", "ゴア", "シャガル", "透明"};
...
void PalicoFurCoatChange(MenuEntry *entry) {
    u8 kenami;
    std::vector<std::string> listPalicoFurCoat{"アイルー", "メラルー", "アメショ", "ワントーン", "ツートーン", "ミケ", "ハンテン"};
...
void PalicoEarChange(MenuEntry *entry) {
    u8 mimi;
    std::vector<std::string> listPalicoEar{"ふつう", "たれ耳", "聞き耳", "立ち耳", "まる耳", "透明"};
...
void PalicoTailChange(MenuEntry *entry) {
    u8 tail;
    std::vector<std::string> listPalicoTail{"ふつう", "ダンゴ", "カギ", "ふさふさ", "ながまる", "透明"};
...
void PalicoNameChange(MenuEntry *entry) {
    ...
    if (choice == 1) {
        ...
        if (keyboard.Open(namekbd) != -1) {
            ...
            Process::WriteString(palicoChoice * 0x494 + 0x8338AFE, namekbd, StringFormat::Utf8);
        }
    }
}
...
void PalicoCommentChange(MenuEntry *entry) {
    ...
    if (choice == 1) {
        ...
        if (keyboard.Open(namekbd) != -1) {
            ...
            Process::WriteString(palicoChoice * 0x494 + 0x8338920, namekbd, StringFormat::Utf8);
        }
    }
}
...
void PalicoGodParentChange(MenuEntry *entry) {
    ...
    if (choice == 1) {
        ...
        if (keyboard.Open(namekbd) != -1) {
            ...
            Process::WriteString(palicoChoice * 0x494 + 0x833895C, namekbd, StringFormat::Utf8);
        }
    }
}
...
void PalicoPredecessorHusbandChange(MenuEntry *entry) {
    ...
    if (choice == 1) {
        ...
        if (keyboard.Open(namekbd) != -1) {
            ...
            Process::WriteString(palicoChoice * 0x494 + 0x833897C, namekbd, StringFormat::Utf8);
        }
    }
}
...
void AttackPowerChange(MenuEntry *entry) { Process::Write16(0x831B450, attackPoint); }
...
void DefencePowerChange(MenuEntry *entry) { Process::Write16(0x831B45E, defencePoint); }
...
void AttributeChange(MenuEntry *entry) { Process::Write16(0x831B45A, attributePoint); }

void ResistanceOption(MenuEntry *entry) {
    Keyboard keyboard("グループを選んでください。", {"火耐性変更", "水耐性変更", "雷耐性変更", "氷耐性変更", "龍耐性変更", "一括変更"});
    int choice = keyboard.Open();
    if (choice == 0) {
...
void RGBOutput(MenuEntry *entry) {
    Keyboard keyboard("グループを選んでください。", {"入力された値を確認", "出力された色を確認"});
    int choice = keyboard.Open();
    if (choice == 0) {
...
void MealWaterResistanceUp(MenuEntry *entry) { Process::Write8(0x83A6E6D, 127); }

void MealThunderResistanceUp(MenuEntry *entry) { Process::Write8(0x83A6E6E, 127); }

void MealIceResistanceUp(MenuEntry *entry) { Process::Write8(0x83A6E6F, 127); }

void MealDragonResistanceUp(MenuEntry *entry) { Process::Write8(0x83A6E70, 127); }
...
void LocalTimeDisplay(MenuEntry *entry) {
    ...
    std::snprintf(time_string, 30, "%2i:%02i %s", (calendar_time.tm_hour % 12) == 0 ? 12 : (calendar_time.tm_hour % 12), calendar_time.tm_min,
                  (calendar_time.tm_hour / 12) ? "PM" : "AM");
