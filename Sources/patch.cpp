...
void CreateDefaultFile(MenuEntry* /*entry*/) {
  const Keyboard kKeyboard("patch.binを初期化しますか？", {"はい", "いいえ"});
  const int kChoice = kKeyboard.Open();
  if (kChoice == 0) {
    CreateDefaultFile();
  }
}
...
void PatchProcessUiMainTextColorEditor(MenuEntry* /*entry*/) {
  ...
  File::Open(file, kPathToBin);
  file.Read(reinterpret_cast<char*>(&color), sizeof(color));
  const Keyboard kKeyboard("どの値を変更しますか？", {"R", "G", "B"});
  u8 value = 0;
  const int kChoice = kKeyboard.Open();
  if (kChoice == 0 && kKeyboard.Open(value) == 0) {
    color.ui_main_text_r = value;
  } else if (kChoice == 1 && kKeyboard.Open(value) == 0) {
    color.ui_main_text_g = value;
  } else if (kChoice == 2 && kKeyboard.Open(value) == 0) {
...
void PatchProcessCustomKeyboardBackgroundBorderColorEditor(
    MenuEntry* /*entry*/) {
  const std::string kPathToBin = GetPathToBin();
  File file;
  if (File::Exists(kPathToBin) == 0) {
...
void PatchProcessDefaultTheme(MenuEntry* /*entry*/) {
  const Keyboard kKeyboard("CTRPFの色を元に戻しますか？", {"はい", "いいえ"});
  const int kChoice = kKeyboard.Open();
  if (kChoice == 0) {
